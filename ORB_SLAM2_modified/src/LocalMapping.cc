...
void LocalMapping::Run()
{
	...
	//等待处理的关键帧列表不为空
	    ...
	    // 剔出在ProcessNewKeyFrame由于新插入当前帧而引进的MapPoint中那些不好的点
            MapPointCulling();
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // 步骤3:跟踪局部地图过程(Tracking线程)中新匹配上的MapPoint和当前帧绑定
    // 在TrackＬocalMap(跟踪局部地图)函数中将局部地图中的MapPoint与当前帧进行了匹配，但没有对这些匹配上的MapPoint与当前帧进行关联
    const vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();//得到当前帧所有匹配上的特征点
    ...
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        ...
        if(pMP)
        {
            ...
            {
	        //非当前帧生成的MapPoint
	        //为当前帧在tracking过程中跟踪到的MapPoint更新属性，跟踪到的局部地图的特征点
                if(!pMP->IsInKeyFrame(mpCurrentKeyFrame))
                {
		    //为该MapPoint添加当前帧的观测
                    pMP->AddObservation(mpCurrentKeyFrame, i);
		    //获得该MapPoint的平均方向和观测距离范围
	...
	// 步骤3：判断相机运动的基线是不是足够长
	  ...
	  // baseline与景深的比例
	   ...
	   // 如果特别远(比例特别小)，那么不考虑当前邻接的关键帧，不生成3D点
        ...
        // 步骤4：根据两个关键帧的位姿计算它们之间的基本矩阵,这里是通过位姿来计算F矩阵
...
void LocalMapping::SearchInNeighbors()
{
    ...
    for(vector<KeyFrame*>::const_iterator vit=vpNeighKFs.begin(), vend=vpNeighKFs.end(); vit!=vend; vit++)
    {
        ...
        //选取与当前关键帧共视的关键帧的前5个共视关键帧，称为二级相邻帧
        ...
        for(vector<KeyFrame*>::const_iterator vit2=vpSecondNeighKFs.begin(), vend2=vpSecondNeighKFs.end(); vit2!=vend2; vit2++)
        {
            ...
            if(pKFi2->isBad() || pKFi2->mnFuseTargetForKF==mpCurrentKeyFrame->mnId || pKFi2->mnId==mpCurrentKeyFrame->mnId)
                continue;
            vpTargetKFs.push_back(pKFi2);//存入二级相邻帧率
        }
    }
    ...
    ORBmatcher matcher;
    // 步骤2：将当前帧的MapPoints分别与一级二级相邻帧(的MapPoints)进行融合,当前帧的MapPoint投影到相邻帧
    vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)
    {
        KeyFrame* pKFi = *vit;
	// 投影当前帧的MapPoints到相邻关键帧pKFi中，并判断是否有重复的MapPoints
	//1.如果MapPoint能匹配关键帧的特征点，并且该点有对应的MapPoint,那么将俩个MapPoint合并（选择观测数最多的）
    ...
    // 步骤3：将一级二级相邻帧的MapPoints分别与当前帧（的MapPoints）进行融合，相邻帧的MapPoint投影到当前帧
    // 遍历每一个一级邻接和二级邻接关键帧
