...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        // Loopclosing中的关键帧是LocalMapping发送过来的，ＬocalMapping是Tracking中发过来的
        // 在LocalMapping中通过InserKeyFrame将关键帧插入到闭环检测队列mlpLoopKeyFrameQueue
...
bool LoopClosing::DetectLoop()
{
    ...
    if(mpCurrentKF->mnId<mLastLoopKFid+10)//闭环检测不能太频繁了
    {
    ...
    // 步骤2：遍历所有共视关键帧，计算当前关键帧与每个共视关键帧的bow相似度得分，并得到最低得分minScore
...
bool LoopClosing::ComputeSim3()
{
	...
	// 防止在LocalMapping中KeyFrameCulling函数将此关键帧作为冗余帧剔除
        ...
        //步骤2:将当前帧mpCurrentKF与候选关键帧pKF匹配
    ...
    // 一直循环所有经过特征点匹配筛选的候选帧，每个候选帧迭代5次，如果5次迭代后得不到结果，就换下一个候选帧
    ...
    while(nCandidates>0 && !bMatch)
    {
		...
		//查找更多的匹配(成功的闭环匹配需要满足足够多的匹配特征点数，之前使用SearchByBoW进行特征点匹配会有漏匹配)
    ...
    // 步骤6:取出闭环匹配上关键帧的相连关键帧，得到它们的MapPoint放入mvpLoopMapPoints，其中mpMatchedKF为匹配上的闭环关键帧
