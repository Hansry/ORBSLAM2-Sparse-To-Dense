...
/**
 * @brief bundle adjustment Optimazation
 * 3D-2D最小化重投影误差 e=(u,v)-project(Tcw*Pw)
 * 
 * 1.Vertex:g2o::VertexSE3Expmap(),即当前帧的Ｔcw
 * 　　　　　 g2o::VertexSBAPointXYZ()，MapPoint的mWorldPos
 * 
 * 2.Edge:
 *   -g2o::EdgeSE3ProjectXYZ(),BaseBinaryEdge
 *      + Vertex:待优化当前帧的Ｔcw
 *      + Vertex:待优化MapPoint的基于世界坐标系的坐标
 *      + meansurement:MapPoint在当前帧中的二维位置(u,v)
...
/**
 * @brief Pose only Optimiation
 * 
 * 3D-2D最小化重投影误差e=（u,v）-project(Tcw*Pw) \n
 * 只优化Frame的Ｔcw,不优化MapPoints的坐标
 * 
 * 1.Vertex:g2o::VertexSim3Expmap(),即当前帧的Tcw（相机相对于世界坐标系的相对位姿）
 * 2.Edge:
 *   -g2o::EdgeSE3ProjectXYZOnlyPose(),BaseUnaryEdge
 *      +Vertex::待优化当前帧的Tcw
 *      +meansurement:MapPoint在当前帧的二维坐标(u,v)
 *      +InfoMatrix:invSigma2(与特征点所在的尺度相关)
 *   -g2o::EdgeStereoSE3ProjectXYZOnlyPose(),BaseUnaryEdge
 *      +Vertex:待优化当前帧的Tcw
 *      +meansurement:MapPoint在当前帧中的二维位置(ul,v,ur)
 *      +InfoMatrix:invSigma2(与特征点所在的尺度有关)
 * 
 * @param   pFrame Frame
 * @return  inliers数量
...
{
    //该优化函数主要用于Tracking线程中：恒速模型运动跟踪（TrackWithMotionModel）、参考帧模型跟踪（TrackReferenceKeyFrame）、
    //地图跟踪（TrackLocalMap）、重定位(Relocalization)
    ...
    for(int i=0; i<N; i++)
    {
	    ...
	    // 单目情况，也有可能在双目下，当前帧的左兴趣点找不到匹配的有兴趣点
            if(pFrame->mvuRight[i]<0)
            {
                ...
                Eigen::Matrix<double,2,1> obs;
                const cv::KeyPoint &kpUn = pFrame->mvKeysUn[i];//取出具有MapPoint的对应图像的特征点
                obs << kpUn.pt.x, kpUn.pt.y;

                g2o::EdgeSE3ProjectXYZOnlyPose* e = new g2o::EdgeSE3ProjectXYZOnlyPose();//建立投影边
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            if(chi2>chi2Mono[it])
            {                
                pFrame->mvbOutlier[idx]=true;// 设置为outlier
                e->setLevel(1);
                nBad++;
            }
            else
            {
                pFrame->mvbOutlier[idx]=false;// 设置为inlier
                e->setLevel(0);
            }
...
/**
 *@brief Local BundleAdjustment
 * 1.vertex:
 *   --g2o::VertexSE3Expmap(),LocalKeyFrames,即当前关键帧、与当前关键帧相连的关键帧的位姿态
 *   --g2o::VertexSE3Expmap(),FixedCCameras,即能观测到LocalMapPoint的关键帧（并不属于LocalKeyFrames）的位姿，在优化中这些关键帧的位姿不变
 *   --g2o::VertexSBAPointXYZ(),LocalMapPoints,即ＬocalKeyFrames能观测到的所有MapPoint的位置
 * 2.Edge:
 *   -g2o::EdgeSE3ProjectXYZ(),BaseBinaryEdge 二元边
 *      +Vertex:关键帧的Tcw,MapPoint的Pw(世界坐标系下MapPoint的位姿)
 *      +meansurement:MapPoint在关键帧中的二维位置(u,v)
 *      +InfoMatrix: invSigma2(与特征点所在的尺度有关)
